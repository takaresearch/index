...
本ページは、外来レターを**再現可能な型**として固定し、上級医へのプレゼンテーション／紹介元への報告／患者説明の整合性を同時に担保することを目的とする。レターは常に、以下の順序で記載する。
...
加えて、患者名は常に **`[PATIENT]`** とし、症例同定性のある要素（日時・施設・地域・職業等）はプレースホルダ（例：`[DATE]`, `[MONTH]`, `[OCCUPATION]`）で管理する。

## 0. 原則

- **診断とプランは常に同じアルゴリズム**（`clinical/algorithm.md`）で回す。  
- **レターの冒頭は Diagnosis と Plan**であり、結論を先に提示する（情報の羅列を目的にしない）。  
- Plan は必ず「次回何をするか」を含み、可能な限り **次回で決着がつく設計**に寄せる。  
- レター本文（MAIN paragraph）は「読者が前提知識なしに追える」よう、要点のみを因果順に配置する。  

## 1. レターテンプレ（共通・コピペ用）

以下は、外来レターを最短で作成するための共通骨格である。`New referral` と `Follow-up` のいずれでも、まずはこの枠で書き、必要に応じてState別テンプレ（後述）へ寄せる。
...
“Temporarily” の記載は、原理的には **Plan のうち「検査待機中（while awaiting investigations）」の暫定方針**である。すなわち、(1)フォロー時期の宣言と、(2)待機期間中に継続すべき保存療法（期限つき）に分解して、上記のPlan箇条書きに統合する。
